...
class AnnotatedFile:
    ...
    def __init__(self, annot_data: Dict, all_labels: List[str]):
        ...
        feats, _ = one_hot_encoding(self.annot_data["annotation"],
                                    ...
                                    self.window,
                                    labels=self.labels)
        self.positive_one_hot = feats.data.swapaxes(0, 1).astype(np.bool)
    ...
    @property
    def uri(self):
        return  get_unique_identifier(self.annot_data)

    def get_frames_around_idx(self, idx):
        #, sample_type="pos"):
        #assert sample_type in ("pos", "neg")
        lower_bound = int(np.floor(idx - self.window.samples(int(self.sample_duration / 2))))
        upper_bound = int(np.floor(idx + self.window.samples(int(self.sample_duration / 2))))
        return lower_bound, upper_bound

class Domain:
    """treat files that have the same domain"""

    def __init__(self, domain_name, annotated_files, all_labels):
    ...
    def _get_file(self, frame_index: int):
        ...
        return onset * window.duration, offset * window.duration, file_uri
...
class Domain_set:
    ...
    def __init__(self, label_mapping, protocol, all_labels: List[str],
            subset: str):
        ...
        self.domain_names = {fin['domain'] for fin in file_set}
        #ipdb.set_trace()
        ...
        for i, domain_name in enumerate(self.domain_names):
            domain_annotated_files = [AnnotatedFile(fin, all_labels) for fin in file_set
                                      if fin['domain'] == domain_name]
            domain = Domain(domain_name, domain_annotated_files, all_labels)
